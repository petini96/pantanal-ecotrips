<template>
  ...
  <q-page v-else-if="pkg" :class="['tour-details-page', `theme-${currentTheme}`]">
    ...
    <div class="page-content q-pa-md q-gutter-y-xl">
      ...
      <q-card class="info-card contact-card" flat bordered v-if="pkg.contact">
        <q-card-section>
          <h2 class="text-h6 q-mt-none flex items-center">
            <q-icon name="mdi-card-account-phone-outline" class="q-mr-sm" />
            <span>{{ t('contact_title') }}</span>
          </h2>
          <div class="contact-items q-mt-md">
            <div class="contact-item" v-if="pkg.contact.name">
              <q-icon name="mdi-account-circle-outline" size="22px" />
              <span>{{ pkg.contact.name }}</span>
            </div>
            <a
              v-if="pkg.contact.phone"
              class="contact-item contact-item--link"
              :href="`https://wa.me/${pkg.contact.phone.replace(/\D/g, '')}`"
              target="_blank"
            ...
            >
              ...
              <span>{{ pkg.contact.phone }}</span>
            </a>
            <a
              v-if="pkg.contact.email"
              class="contact-item contact-item--link"
              :href="`mailto:${pkg.contact.email}`"
            >
              <q-icon name="mdi-email-outline" size="22px" />
              <span>{{ pkg.contact.email }}</span>
            </a>
          </div>
        </q-card-section>
      </q-card>
